...
class P_RNet3D(nn.Module):
    ...
    def forward(self, inputs):
        # inputs:  torch.Size([1, 4, 64, 64, 64])
        out_blk1 = self.block1(inputs)
        # out_blk1:  torch.Size([1, 16, 64, 64, 64])
        out_blk2 = self.block2(out_blk1)
        # out_blk2:  torch.Size([1, 16, 64, 64, 64])
        out_blk3 = self.block3(out_blk2)
        # out_blk3:  torch.Size([1, 16, 64, 64, 64])
        out_blk4 = self.block4(out_blk3)
        # out_blk4:  torch.Size([1, 16, 64, 64, 64])
        out_blk5 = self.block5(out_blk4)
        # out_blk5:  torch.Size([1, 16, 64, 64, 64])
        ...
        # 每个out_blk的shape是[1, 16, 64, 64, 64]，然后在channel维度上下采样4倍，然后concat，所以out_blks的shape是[1, 20, 64, 64, 64]
        # out_blks:  torch.Size([1, 20, 64, 64, 64])
                              
        out_logits = self.block6(out_blks)
        # out_logits:  torch.Size([1, 2, 64, 64, 64])
...
def build_model():
    model = P_RNet3D(c_in=4, 
                     c_blk=16,
                     n_classes=2,
                     init_weights=True).to('cpu')
    return model

if __name__ == "__main__":
    model = build_model()
    t = torch.rand(1, 4, 64, 64, 64)
    print(t.shape)
    out = model(t)
